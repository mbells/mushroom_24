...
def read_state(state):
    while arduino_serial1 and arduino_serial1.in_waiting:
        state = arduino_serial1.read()[0]
    return state


def write_to_strip():
    if arduino_serial1:
        arduino_serial1.write(cobs.encode(buffer) + b"\0")
    time.sleep(0.1)
...
def update_LEDs(state):
    if state == 0:
        for i in range(NUM_PIXELS):
            set_pixel(i, (20, 20, 20))
    elif state == 1:
        for i in range(NUM_PIXELS):
            set_pixel(i, (int(255.0 * i / NUM_PIXELS), 0, 0))
    elif state == 2:
        for i in range(NUM_PIXELS):
            set_pixel(NUM_PIXELS - i - 1, (0, int(255.0 * i / NUM_PIXELS), 0))
    elif state == 3:
        for i in range(NUM_PIXELS):
            set_pixel(
                i,
